...
def verify_email(payload: auth.VerifyEmailRequest, db: Session = Depends(database.get_db)):
    ...
    if user.verification_expires_at and datetime.utcnow() > user.verification_expires_at.replace(tzinfo=None):
...
def resend_verification(
    ...
    background_tasks.add_task(utils.send_verification_email, payload.email, code, user.f_name)
...
def reset_password(
    payload: auth.ResetPasswordRequest,
    db: Session = Depends(database.get_db)
):
    user = db.query(models.User).filter(models.User.email == payload.email).first()
    if not user:
